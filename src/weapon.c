...
static void give_may_advance_msg(int skill) {
    You_feel("more confident in your %sskills.", skill == P_NONE ? "" : skill <= P_LAST_WEAPON ? "weapon " : skill <= P_LAST_SPELL ? "spell casting " : "fighting ");
}
...
static struct obj * oselect(struct monst *mtmp, int x) {
    ...
    for (otmp = mtmp->minvent; otmp; otmp = otmp->nobj) {
        ...
        if (otmp->otyp == x && !((x == CORPSE || x == EGG) && !touch_petrifies(&mons[otmp->corpsenm])) && (!otmp->oartifact || touch_artifact(otmp, mtmp)))
            return otmp;
    }
    return (struct obj *)0;
}
...
struct obj * select_rwep(struct monst *mtmp) {
    ...
    if (dist2(mtmp->mx, mtmp->my, mtmp->mux, mtmp->muy) <= 13 && couldsee(mtmp->mx, mtmp->my)) {
        for (i = 0; i < SIZE(pwep); i++) {
             ...
             */
            if (((strongmonst(mtmp->data) && (mtmp->misc_worn_check & W_ARMS) == 0) || !objects[pwep[i]].oc_big) && (objects[pwep[i]].oc_material != SILVER || !hates_silver(mtmp->data))) {
    ...
    for (i = 0; i < SIZE(rwep); i++) {
        ...
        if (rwep[i] == DART && !likes_gems(mtmp->data) && m_carrying(mtmp, SLING)) { /* propellor */
            for (otmp = mtmp->minvent; otmp; otmp = otmp->nobj) {
        ...
        if (prop < 0) {
            ...
            if ((otmp = MON_WEP(mtmp)) && otmp->cursed && otmp != propellor && mtmp->weapon_check == NO_WEAPON_WANTED)
                propellor = 0;
        }
        ...
        if (propellor != 0) {
            ...
            if (rwep[i] != LOADSTONE) {
                ...
                if ((otmp = oselect(mtmp, rwep[i])) && !otmp->oartifact && (!otmp->cursed || otmp != MON_WEP(mtmp)))
                    return (otmp);
            } else {
                for (otmp = mtmp->minvent; otmp; otmp = otmp->nobj) {
...
struct obj * select_hwep(struct monst *mtmp) {
    ...
    for (otmp = mtmp->minvent; otmp; otmp = otmp->nobj) {
        if (otmp->oclass == WEAPON_CLASS && otmp->oartifact && touch_artifact(otmp, mtmp) && ((strong && !wearing_shield) || !objects[otmp->otyp].oc_big))
            return otmp;
    }
    ...
    for (i = 0; i < SIZE(hwep); i++) {
        if (hwep[i] == CORPSE && !(mtmp->misc_worn_check & W_ARMG))
            continue;
        if (((strong && !wearing_shield) || !objects[hwep[i]].oc_big) && (objects[hwep[i]].oc_material != SILVER || !hates_silver(mtmp->data)))
            Oselect(hwep[i]);
    }
    ...
    if (obj && obj != &zeroobj) {
        ...
        if (canseemon(mon)) {
            ...
            if (obj->cursed && obj->otyp != CORPSE) {
                ...
                pline("%s %s to %s%s %s!", the_object_welds, is_plural(obj) ? "themselves" : "itself", monster_name, possessive_suffix(monster_name), mbodypart(mon, HAND));
                obj->bknown = 1;
            }
        }
...
static bool could_advance(int skill) {
    return !P_RESTRICTED(skill) && P_SKILL(skill) < P_MAX_SKILL(skill) && ((P_ADVANCE(skill) >= (unsigned)practice_needed_to_advance(P_SKILL(skill)) && u.skills_advanced < P_SKILL_LIMIT));
}
...
static bool peaked_skill(int skill) {
    return !P_RESTRICTED(skill) && P_SKILL(skill) >= P_MAX_SKILL(skill) && ((P_ADVANCE(skill) >= (unsigned)practice_needed_to_advance(P_SKILL(skill))));
}
...
static void skill_advance(int skill) {
    ...
    P_SKILL(skill) >= P_MAX_SKILL(skill) ? "most" : "more", P_NAME(skill));
}
        ...
        if (want_dump) {
            ...
            if (eventually_advance > 0 || maxxed_cnt > 0) {
                ...
                if (eventually_advance > 0) {
                    sprintf(buf, "(Skill%s flagged by \"*\" may be enhanced %s.)", plur(eventually_advance), (u.ulevel < MAXULEV) ? "when you're more experienced" : "if skill slots become available");
                    add_menu(win, NO_GLYPH, &any, 0, 0, ATR_NONE, buf, MENU_UNSELECTED);
                }
                if (maxxed_cnt > 0) {
                    sprintf(buf, "(Skill%s flagged by \"#\" cannot be enhanced any further.)", plur(maxxed_cnt));
                    add_menu(win, NO_GLYPH, &any, 0, 0, ATR_NONE, buf, MENU_UNSELECTED);
                }
            ...
            for (i = skill_ranges[pass].first; i <= skill_ranges[pass].last; i++) {
                ...
                if (i == skill_ranges[pass].first) {
                    if (want_dump) {
                        ...
                        add_menu(win, NO_GLYPH, &any, 0, 0, iflags.menu_headings, skill_ranges[pass].name, MENU_UNSELECTED);
                }
                if (want_dump) {
                    if (P_SKILL(i) > P_UNSKILLED) {
                        sprintf(buf2, "%-*s [%s]", longest, P_NAME(i), skill_level_name(i, buf));
                        dump("    ", buf2);
                        logged = true;
                    ...
                    if (wizard) {
                        if (!iflags.menu_tab_sep)
                            sprintf(buf, " %s%-*s %-12s %5d(%4d)", prefix, longest, P_NAME(i), sklnambuf, P_ADVANCE(i), practice_needed_to_advance(P_SKILL(i)));
                        else
                            sprintf(buf, " %s%s\t%s\t%5d(%4d)", prefix, P_NAME(i), sklnambuf, P_ADVANCE(i), practice_needed_to_advance(P_SKILL(i)));
                    } else {
                        if (!iflags.menu_tab_sep)
                            sprintf(buf, " %s %-*s [%s]", prefix, longest, P_NAME(i), sklnambuf);
                        else

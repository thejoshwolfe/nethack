    ...
    if(nearby && mdat->msound == MS_BRIBE &&
        ...
        if (mtmp->mux != u.ux || mtmp->muy != u.uy) {
            ...
            pline("%s whispers at thin air.",
                    cansee(mtmp->mux, mtmp->muy) ? name : "It");
        ...
        for(m2=fmon; m2; m2 = nmon) {
            ...
            if ((telepathic(m2->data) &&
                ...
                if (cansee(m2->mx, m2->my)) {
                    char name[BUFSZ];
                    mon_nam(name, BUFSZ, m2);
                    pline("It locks on to %s.", name);
                }
                m2->mhp -= rnd(15);
                if (m2->mhp <= 0)
...
bool itsstuck (struct monst *mtmp) {
    if (sticks(youmonst.data) && mtmp==u.ustuck && !u.uswallow) {
        char name[BUFSZ];
        Monnam(name, BUFSZ, mtmp);
        pline("%s cannot escape from you!", name);
        return(true);
    }
    return(false);
}
        ...
        if(mmoved == 1) {
            ...
            if(IS_DOOR(levl[mtmp->mx][mtmp->my].typ)
                ...
                if (flags.verbose && canseemon(mtmp)) {
                    char name[BUFSZ];
                    Monnam(name, BUFSZ, mtmp);
                    Norep("%s %s %s the iron bars.", name,

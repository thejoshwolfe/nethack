...
void amulet(void) {
    ...
    if ((((amu = uamul) != 0 && amu->otyp == AMULET_OF_YENDOR) || ((amu = uwep) != 0 && amu->otyp == AMULET_OF_YENDOR)) && !rn2(15)) {
        for (ttmp = ftrap; ttmp; ttmp = ttmp->ntrap) {
            if (ttmp->ttyp == MAGIC_PORTAL) {
                int du = distu(ttmp->tx, ttmp->ty);
                char it_feels[BUFSZ];
                Tobjnam(it_feels, BUFSZ, amu, "feel");
                if (du <= 9)
                    pline("%s hot!", it_feels);
                else if (du <= 64)
                    pline("%s very warm.", it_feels);
                else if (du <= 144)
                    pline("%s warm.", it_feels);
 ...
 */
static bool mon_has_arti(struct monst *mtmp, short otyp) {
    struct obj *otmp;
...
static bool you_have(int mask) {
    switch (mask) {
        case M3_WANTSAMUL:
            return (bool)(u.uhave.amulet);
        case M3_WANTSBELL:
            return (bool)(u.uhave.bell);
        case M3_WANTSCAND:
            return (bool)(u.uhave.menorah);
        case M3_WANTSBOOK:
            return (bool)(u.uhave.book);
        case M3_WANTSARTI:
            return (bool)(u.uhave.questart);
        default:
            break;
    }
    return (0);
...
int tactics(struct monst *mtmp) {
    ...
    switch (strat) {
        case STRAT_HEAL: /* hide and recover */
            ...
            if (In_W_tower(mtmp->mx, mtmp->my, &u.uz) || (mtmp->iswiz && !xupstair && !mon_has_amulet(mtmp))) {
        ...
        default: /* kill, maim, pillage! */
        {
            ...
            signed char tx = STRAT_GOALX(strat), ty = STRAT_GOALY(strat);
                ...
                if (!MON_AT(tx, ty) || (mtmp->mx == tx && mtmp->my == ty)) {
                    ...
                    if ((otmp = on_ground(which_arti(targ))) != 0) {
                        if (cansee(mtmp->mx, mtmp->my)) {
                            char subject[BUFSZ];
                            Monnam(subject, BUFSZ, mtmp);

                            char direct_object[BUFSZ];
                            if (distu(mtmp->mx, mtmp->my) <= 5)
                                strcpy(direct_object, doname(otmp));
                            else
                                strcpy(direct_object, distant_name(otmp, doname));

                            pline("%s picks up %s.", subject, direct_object);
                        }
...
void clonewiz(void) {
    ...
    if ((mtmp2 = makemon(&mons[PM_WIZARD_OF_YENDOR], u.ux, u.uy, NO_MM_FLAGS)) != 0) {
    ...
    if (!rn2(10) && Inhell) {
        ...
        for (i = rnd(tmp); i > 0; --i)
            for (j = 0; j < 20; j++) {
                ...
                } while (mcast && attacktype(&mons[makeindex], AT_MAGC) && monstr[makeindex] >= monstr[mcast->mnum]);
                /* do this after picking the monster to place */
                if (mcast && !enexto(&bypos, mcast->mux, mcast->muy, &mons[makeindex]))
                    continue;
                ...
                if (mtmp && (mtmp->data->maligntyp == 0 || sgn(mtmp->data->maligntyp) == sgn(castalign))) {
                    count++;
                    break;
                }
            }
...
void resurrect(void) {
    ...
    if (!flags.no_of_wizards) {
        ...
        while ((mtmp = *mmtmp) != 0) {
            ...
            !mon_has_amulet(mtmp) && (elapsed = monstermoves - mtmp->mlstmv) > 0L) {
...
void cuss(struct monst *mtmp) {
    if (mtmp->iswiz) {
        if (!rn2(5)) {
            /* typical bad guy action */
            char name[BUFSZ];
            Monnam(name, BUFSZ, mtmp);
            pline("%s laughs fiendishly.", name);
        } else if (u.uhave.amulet && !rn2(SIZE(random_insult))) {
            verbalize("Relinquish the amulet, %s!", random_insult[rn2(SIZE(random_insult))]);
        } else if (u.uhp < 5 && !rn2(2)) {
            /* Panic */
            verbalize(rn2(2) ? "Even now thy life force ebbs, %s!" : "Savor thy breath, %s, it be thy last!", random_insult[rn2(SIZE(random_insult))]);
        } else if (mtmp->mhp < 5 && !rn2(2)) {
            /* Parthian shot */
            verbalize(rn2(2) ? "I shall return." : "I'll be back.");
        } else {
            verbalize("%s %s!", random_malediction[rn2(SIZE(random_malediction))], random_insult[rn2(SIZE(random_insult))]);
        }
        ...
        if (!rn2(5)) {
            char name[BUFSZ];
            Monnam(name, BUFSZ, mtmp);
            pline("%s casts aspersions on your ancestry.", name);
        } else {

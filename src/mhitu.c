            ...
            && !mtmp->mcan && !mtmp->mspec_used)
    {
        ...
        switch (mattk->aatyp) {
            ...
            case AT_BUTT:
                ...
                break;
            case AT_TUCH:
                pline("%s touches you!", name);
                break;
            case AT_TENT:
                pline("%s%s tentacles suck you!", name, possessive_suffix(name));
                break;
            case AT_EXPL:
            case AT_BOOM:
                pline("%s explodes!", name);
                break;
            default:
                pline("%s hits!", name);
        }
...
/* monster missed you */
static void missmu (struct monst *mtmp, bool nearmiss, struct attack *mattk) {
    if (!canspotmon(mtmp))
    ...
    if (could_seduce(mtmp, &youmonst, mattk) && !mtmp->mcan) {
        pline("%s pretends to be friendly.", name);
    } else {
        if (!flags.verbose || !nearmiss)
            pline("%s misses.", name);
        else
            pline("%s just misses!", name);
    }
    stop_occupation();
}
...
static void wildmiss ( struct monst *mtmp, struct attack *mattk) {
    ...
    if (!mtmp->mcansee || (Invis && !perceives(mtmp->data))) {
        ...
        if (compat) {
            pline("%s tries to touch you and misses!", attacker);
        } else {
            switch(rn2(3)) {
                case 0: pline("%s %s wildly and misses!", attacker, swings);
                        break;
                case 1: pline("%s attacks a spot beside you.", attacker);
                        break;
                case 2: pline("%s strikes at %s!", attacker,
                                levl[mtmp->mux][mtmp->muy].typ == WATER
                                ? "empty water" : "thin air");
                        break;
                default:pline("%s %s wildly!", attacker, swings);
                        break;
            }
        }
        ...
        if (compat)
            pline("%s reaches towards your distorted image.",attacker);
        else
            pline("%s is fooled by water reflections and misses!",attacker);

    } else {
        impossible("%s attacks you without knowing your location?", attacker);
    }
}

// struct permonst *mdat, /* if mtmp is polymorphed, mdat != mtmp->data */
void expels ( struct monst *mtmp, struct permonst *mdat, bool message) {
    if (message) {
        if (is_animal(mdat))
            ...
            if (i >= NATTK || mdat->mattk[i].aatyp != AT_ENGL)
                ...
                You("get expelled from %s%s!", name, blast);
            }
        }
    }
...
struct attack * getmattk (struct permonst *mptr, int indx,
        int prev_result[], struct attack *alt_attk_buf)
{
...
int mattacku (struct monst *mtmp) {
    ...
    if (u.uundetected && !range2 && foundyou && !u.uswallow) {
        ...
        if (is_hider(youmonst.data)) {
            ...
            if (enexto(&cc, u.ux, u.uy, youmonst.data)) {
                ...
                char name[BUFSZ];
                Monnam(name, BUFSZ, mtmp);
                pline("%s is killed by a falling %s (you)!", name, youmonst.data->mname);
                killed(mtmp);
                newsym(u.ux,u.uy);
            ...
            if (obj && is_metallic(obj)) {
                char name[BUFSZ];
                mon_nam(name, BUFSZ, mtmp);
                Your("blow glances off %s%s helmet.", name, possessive_suffix(name));
            } else {
                char name[BUFSZ];
                Monnam(name, BUFSZ, mtmp);
                if (3 + find_mac(mtmp) <= rnd(20)) {
                    pline("%s is hit by a falling piercer (you)!", name);
                    if ((mtmp->mhp -= d(3,6)) < 1)
                        killed(mtmp);
                } else {
                    pline("%s is almost hit by a falling piercer (you)!", name);
                }
            }
        } else {
            if (!youseeit) {
                ...
                if (obj || (youmonst.data->mlet == S_EEL && is_pool(u.ux, u.uy))) {
                    ...
                    if (youmonst.data->mlet == S_EEL)
                        pline("Wait, %s!  There's a hidden %s named %s there!",
                                name, youmonst.data->mname, plname);
                    else
            ...
            !range2 && foundyou && !u.uswallow)
    {
        if (!youseeit) {
            ...
            char name[BUFSZ];
            m_monnam(name, BUFSZ, mtmp);
            pline("Wait, %s!  That's a %s named %s!", name, youmonst.data->mname, plname);
        }
        u.ustuck = mtmp;
    ...
    if (u.uinvulnerable) {
        ...
        Monnam(name, BUFSZ, mtmp);
        if (mtmp == u.ustuck)
            pline("%s loosens its grip slightly.", name);
        else if(!range2) {
            if (youseeit || sensemon(mtmp))
                pline("%s starts to attack you, but pulls back.", name);
            else
            ...
            case AT_ENGL:
                if (!range2) {
                    if(foundyou) {
                        ...
                        char name[BUFSZ];
                        Monnam(name, BUFSZ, mtmp);
                        pline("%s gulps some air!", name);
                    } else {
                        if (youseeit) {
                            char name[BUFSZ];
                            Monnam(name, BUFSZ, mtmp);
                            pline("%s lunges forward and recoils!", name);
                        } else {
...
static bool u_slip_free (struct monst *mtmp, struct attack *mattk) {
    ...
    if(mtmp->mundetected && (hides_under(mdat) || mdat->mlet == S_EEL)) {
        ...
        if (!(Blind ? Blind_telepat : Unblind_telepat)) {
            ...
            if ((obj = level.objects[mtmp->mx][mtmp->my]) != 0) {
                ...
                pline("%s was hidden under %s!", name, what);
            }
            newsym(mtmp->mx, mtmp->my);
        }
    }
        ...
        case AD_SLEE:
            ...
            if (uncancelled && multi >= 0 && !rn2(5)) {
                ...
                if (Blind) {
                    ...
                    char name[BUFSZ];
                    mon_nam(name, BUFSZ, mtmp);
                    You("are put to sleep by %s!", name);
                }
            }
            break;
        case AD_BLND:
...
dopois:
            ...
            if (uncancelled && !rn2(8)) {
                ...
                Monnam(name, BUFSZ, mtmp);
                sprintf(buf, "%s%s %s",
                        name, possessive_suffix(name), mpoisons_subj(mtmp, mattk));
            ...
            if (!uarmh || uarmh->otyp != DUNCE_CAP) {
                ...
                if (ABASE(A_INT) <= ATTRMIN(A_INT)) {
                    ...
                    while(1) {
                        ...
                        if (lifesaved && (flags.explore || flags.debug)) {
        ...
        case AD_LEGS:
            {
                ...
                if ( u.usteed || Levitation || Flying) {
                    pline("%s tries to reach your %s %s!", attacker,
                            sidestr, body_part(LEG));
                    dmg = 0;
                } else if (mtmp->mcan) {
                    pline("%s nuzzles against your %s %s!", attacker,
                            sidestr, body_part(LEG));
                    dmg = 0;
                } else {
                    if (uarmf) {
                        if (rn2(2) && (uarmf->otyp == LOW_BOOTS || uarmf->otyp == IRON_SHOES))
                            pline("%s pricks the exposed part of your %s %s!",
                                    attacker, sidestr, body_part(LEG));
                        else if (!rn2(5))
                            pline("%s pricks through your %s boot!", attacker, sidestr);
                        else {
                            pline("%s scratches your %s boot!", attacker, sidestr);
                            dmg = 0;
                            break;
                        }
                    } else {
                        pline("%s pricks your %s %s!", attacker, sidestr, body_part(LEG));
                    }
        ...
        case AD_STON:       /* cockatrice */
            ...
            if(!rn2(3)) {
                char attacker[BUFSZ];
                mon_nam(attacker, BUFSZ, mtmp);
                if (mtmp->mcan) {
                    if (flags.soundok)
                        You_hear("a cough from %s!", attacker);
                } else {
                    if (flags.soundok)
...
do_stone:
            ...
            if ((!mtmp->mcan || u.ustuck == mtmp) && !sticks(youmonst.data)) {
                ...
                if (!u.ustuck && !rn2(10)) {
                    if (u_slip_free(mtmp, mattk)) {
                        ...
                        pline("%s swings itself around you!", attacker);
                        u.ustuck = mtmp;
                    }
                } else if(u.ustuck == mtmp) {
                    ...
                    dmg = 0;
                    if(flags.verbose)
                        pline("%s brushes against your %s.", attacker, body_part(LEG));
                }
            } else dmg = 0;
            break;
        case AD_WERE:
                ...
                default:
                    ...
                    if (is_animal(mtmp->data) && *buf) {
                        if (canseemon(mtmp)) {
                            char name[BUFSZ];
                            Monnam(name, BUFSZ, mtmp);
                            pline("%s tries to %s away with %s.", name,
        ...
        case AD_HEAL:
            ...
            if (!uwep && !uarmu && !uarm && !uarmh && !uarms && !uarmg && !uarmc && !uarmf) {
                ...
                char name[BUFSZ];
                Monnam(name, BUFSZ, mtmp);
                pline("%s hits!  (I hope you don't mind.)", name);
                if (Upolyd) {
                    u.mh += rnd(7);
                    if (!rn2(7)) {
        ...
        case AD_DETH:
            {
                char name[BUFSZ];
                Monnam(name, BUFSZ, mtmp);
                pline("%s reaches out with its deadly touch.", name);
            }
        ...
        case AD_PEST:
            {
                char name[BUFSZ];
                Monnam(name, BUFSZ, mtmp);
                pline("%s reaches out, and you feel fever and chills.", name);
            }
        ...
        case AD_FAMN:
            {
                char name[BUFSZ];
                Monnam(name, BUFSZ, mtmp);
                pline("%s reaches out, and your body shrivels.", name);
            }
            exercise(A_CON, false);
            if (!is_fainted())
...
static int gulpmu ( struct monst *mtmp, struct attack *mattk) {
    ...
    if (!u.uswallow) {      /* swallows you */
        ...
        if (is_animal(mtmp->data) && u.usteed) {
            ...
            pline("%s lunges forward and plucks you off %s!", attacker, steed_name);
            dismount_steed(DISMOUNT_ENGULFED);
        } else {
            pline("%s engulfs you!", attacker);
        }
        stop_occupation();
        ...
        if (i > 0) {
            ...
            char snap[BUFSZ];
            vtense(snap, BUFSZ, s, "snap");
            pline_The("%s %s loose.", s, snap);
            unleash_all();
        }
    ...
    switch(mattk->adtyp) {
        ...
        case AD_DGST:
            if (Slow_digestion) {
                ...
                char name[BUFSZ];
                Monnam(name, BUFSZ, mtmp);
                pline("%s totally digests you!", name);
                tmp = u.uhp;
                if (Half_physical_damage) tmp *= 2; /* sorry */
            } else {
                char name[BUFSZ];
                Monnam(name, BUFSZ, mtmp);
                pline("%s%s digests you!", name,
    ...
    if (touch_petrifies(youmonst.data) && !resists_ston(mtmp)) {
        char name[BUFSZ];
        Monnam(name, BUFSZ, mtmp);
        pline("%s very hurriedly %s you!", name,
                    ...
                    (dmgtype(mtmp->data, AD_DGST) && Slow_digestion)))
        {
            char name[BUFSZ];
            mon_nam(name, BUFSZ, mtmp);
            pline("Obviously %s doesn't like your taste.", name);
        }
        expels(mtmp, mtmp->data, false);
    }
    return(1);
...
static int explmu ( struct monst *mtmp, struct attack *mattk, bool ufound) {
    ...
    switch(mattk->adtyp) {
        case AD_STON:
            ...
            if (Reflecting && couldsee(mtmp->mx, mtmp->my) &&
                ...
                if (useeit) {
                    ...
                    Monnam(name, BUFSZ, mtmp);
                    char pname[BUFSZ];
                    s_suffix(pname, BUFSZ, name);
                    ureflects("%s gaze is reflected by your %s.", pname);
                }
                ...
                if (!m_canseeu(mtmp)) { /* probably you're invisible */
                    if (useeit) {
                        char name[BUFSZ];
                        Monnam(name, BUFSZ, mtmp);
                        pline("%s doesn't seem to notice that %s gaze was reflected.", name,
                                mhis(mtmp));
                    }
                    break;
                }
        ...
        case AD_BLND:
            if (!mtmp->mcan && canseemon(mtmp) && !resists_blnd(&youmonst)
                ...
                char pname[BUFSZ];
                monster_possessive(pname, BUFSZ, mtmp);
                You("are blinded by %s radiance!", pname);
            ...
            (is_acid ? obj->oeroded2 : obj->oeroded) < MAX_ERODE)
    {
        if (obj->greased || obj->oerodeproof || (obj->blessed && rn2(3))) {
            if (vis) {
                ...
                pline("Somehow, %s weapon is not affected.", pname);
            }
...
int doseduce (struct monst *mon) {
    ...
    char foocubus_name_cap[BUFSZ];
    Monnam(foocubus_name_cap, BUFSZ, mon);
    if (mon->mcan || mon->mspec_used) {
        pline("%s acts as though %s has got a %sheadache.",
                foocubus_name_cap, mhe(mon), mon->mcan ? "severe " : "");
        return 0;
    }

    if (unconscious()) {
        pline("%s seems dismayed at your lack of response.", foocubus_name_cap);
        return 0;
    }
    ...
    if (Blind) {
        ...
        You_feel("very attracted to %s.", name);
    }
    ...
    char noit_name[BUFSZ];
    noit_mon_nam(noit_name, BUFSZ, mon);
    char noit_name_cap[BUFSZ];
    noit_Monnam(noit_name_cap, BUFSZ, mon);
    pline("Time stands still while you and %s lie in each other's arms...", noit_name);
    if (rn2(35) > ACURR(A_CHA) + ACURR(A_INT)) {
        /* Don't bother with mspec_used here... it didn't get tired! */
        pline("%s seems to have enjoyed it more than you...", noit_name_cap);
        switch (rn2(5)) {
        ...
        You("seem to have enjoyed it more than %s...", noit_name);
        switch (rn2(5)) {
            ...
            case 1: You_feel("good enough to do it again.");
                    ...
                    exercise(A_CON, true);
                    break;
            case 2: You("will always remember %s...", noit_name);
                    (void) adjattrib(A_WIS, 1, true);
                    exercise(A_WIS, true);
                    break;
    ...
    if (mon->mtame) {
        ...
        if (!cost) verbalize("It's on the house!");
            ...
            pline("%s takes %ld %s for services rendered!",
                    noit_name_cap, cost, currency(cost));
            u.ugold -= cost;
            mon->mgold += cost;
        }
    }
...
static int passiveum (struct permonst *olduasmon, struct monst *mtmp, struct attack *mattk) {
    ...
    char name_cap[BUFSZ];
    Monnam(name_cap, BUFSZ, mtmp);
    /* These affect the enemy even if you were "killed" (rehumanized) */
    switch(olduasmon->mattk[i].adtyp) {
        case AD_ACID:
            if (!rn2(2)) {
                pline("%s is splashed by your acid!", name_cap);
                if (resists_acid(mtmp)) {
                    pline("%s is not affected.", name_cap);
                    tmp = 0;
                }
            } else {
                tmp = 0;
        ...
        case AD_STON: /* cockatrice */
            {
                ...
                if (!resists_ston(mtmp) && (protector == 0L ||
                    ...
                    pline("%s turns to stone!", name_cap);
                    stoned = 1;
                    xkilled(mtmp, 0);
    ...
    if (rn2(3)) switch(youmonst.data->mattk[i].adtyp) {
        ...
        case AD_PLYS: /* Floating eye */
            ...
            if (u.umonnum == PM_FLOATING_EYE) {
                        ...
                        (perceives(mtmp->data) || !Invis))
                {
                    if (Blind) {
                        ...
                        if (mon_reflects(mtmp, "Your gaze is reflected by %s %s."))
                            return 1;
                        pline("%s is frozen by your gaze!", name_cap);
                        mtmp->mcanmove = 0;
                        mtmp->mfrozen = tmp;
                        return 3;
                    }
                ...
                pline("%s is frozen by you.", name_cap);
                mtmp->mcanmove = 0;
                mtmp->mfrozen = tmp;
                return 3;
            }
        ...
        case AD_COLD: /* Brown mold or blue jelly */
            if (resists_cold(mtmp)) {
                ...
                pline("%s is mildly chilly.", name_cap);
                golemeffects(mtmp, AD_COLD, tmp);
                tmp = 0;
                break;
            }
            pline("%s is suddenly very cold!", name_cap);
            u.mh += tmp / 2;
        ...
        case AD_FIRE: /* Red mold */
            if (resists_fire(mtmp)) {
                shieldeff(mtmp->mx, mtmp->my);
                pline("%s is mildly warm.", name_cap);
                golemeffects(mtmp, AD_FIRE, tmp);
                tmp = 0;
                break;
            }
            pline("%s is suddenly very hot!", name_cap);
            break;
        case AD_ELEC:
            if (resists_elec(mtmp)) {
                shieldeff(mtmp->mx, mtmp->my);
                pline("%s is slightly tingled.", name_cap);
                golemeffects(mtmp, AD_ELEC, tmp);
                tmp = 0;
                break;
            }
            pline("%s is jolted with your electricity!", name_cap);
            break;
        default: tmp = 0;
                 break;
    }

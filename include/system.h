...
#include <sys/file.h>
# if defined(ULTRIX_PROTO) || defined(__GNUC__)
extern int NDECL(fork);
# else
extern long NDECL(fork);
# endif
#endif /* ULTRIX */
...
#if defined(HPUX) && !defined(_POSIX_SOURCE)
extern long NDECL(getuid);
extern long NDECL(getgid);
extern long NDECL(getpid);
#else
# ifdef POSIX_TYPES
extern pid_t NDECL(getpid);
extern uid_t NDECL(getuid);
extern gid_t NDECL(getgid);
#  ifdef VMS
extern pid_t NDECL(getppid);
#  endif
# else	/*!POSIX_TYPES*/
...
#  endif
#  ifdef VMS
extern int NDECL(getppid);
extern unsigned NDECL(getuid);
extern unsigned NDECL(getgid);
#  endif
